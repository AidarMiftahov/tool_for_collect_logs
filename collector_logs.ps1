...
function Get-WindowsEventLogs {
    param(
        [string]$IP,
        [PSCredential]$Credential,
        [string]$LocalDestination = "C:\LogVisualizer\Windows"
    )
    ...
    try {
        ...
        $LogDir = "$LocalDestination\host_$IP"
        if (Test-Path $LogDir) {
            $counter = 1
            do {
                $LogDir = "$LocalDestination\host_$IP ($counter)"
                $counter++
            } while (Test-Path $LogDir)
        }
        ...
        $allEvents = Invoke-Command -ComputerName $IP -Credential $Credential -ScriptBlock {
            ...
            return @{
                SystemEvents = & $convert (Get-WinEvent -LogName System -MaxEvents 50)
                ApplicationEvents = & $convert (Get-WinEvent -LogName Application -MaxEvents 50)
                SecurityEvents = & $convert (Get-WinEvent -LogName Security -MaxEvents 20 -ErrorAction SilentlyContinue)
...
function Get-AstraLogs {
    param(
        ...
        [string]$LocalDestination = "C:\LogVisualizer\Linux"
    )
...
foreach ($computer in $networkComputers) {
    ...
    if ($lastOctet -eq 1 -or $lastOctet -eq 255) {
        Write-Host "Skipping reserved IP: $computer" -ForegroundColor Yellow
    ...
    if ($osType -eq "Unknown") {
        Write-Host "Skipping $computer - not Windows or Linux" -ForegroundColor Yellow
